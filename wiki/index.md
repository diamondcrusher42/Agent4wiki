...
- fixes-2026-04-09-post-janitor — Post-Janitor fixes: B1 template injection (WIKI_CONTEXT+ALLOWED_ENDPOINTS), S2 token //3 unified, S3 bridge.py 11 tests, S4 benchmark_score.py 20 tests, S5 synthetic handshake removed. 100/100 tests. Score est. 75+/100. (page not created — see log entry 2026-04-09)
...
- [[fixes-2026-04-10-security-s1s2s3]] — Security fixes (2026-04-10): S1 path traversal (dispatcher.py page_name sanitization), S2 git arg injection (scythe.ts -- separator), S3 shell interpolation (spawner.ts execFileSync). Security C+ → B+. Cleared for testing.
- [[build-state-2026-04-10-v4-phases]] — V4 Phases 2A/2B/2C/2D (2026-04-10): start.sh launcher, 3-tier CLAUDE.md routing (BRAIN_ONLY added), MemPalace auto-query in dispatcher, file handshake as primary. Live: 15% → 60%.
- [[build-state-2026-04-10-v4-live]] — V4 100% live (2026-04-10): Phases 3+4 complete. Bridge delivers directly, session stateless. Pre-live hardening: fail-loud bridge, atomic inbox, worktree cleanup, status.sh. Live test task-live-002: NOTE→Bridge→Telegram confirmed. Thinking bug fixed (MAX_THINKING_TOKENS). claude-mentor skill added.
